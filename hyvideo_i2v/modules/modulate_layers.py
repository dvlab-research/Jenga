...
def modulate(x, shift=None, scale=None, condition_type=None,
    ...
    if condition_type == "token_replace":
        if first_frame_mask is not None:
            # print(x.shape, first_frame_mask.shape, x[:, first_frame_mask].shape, tr_scale.shape, tr_shift.shape)
            x[:, first_frame_mask] = x[:, first_frame_mask] * (1 + tr_scale.unsqueeze(1)) + tr_shift.unsqueeze(1)
            x[:, ~first_frame_mask] = x[:, ~first_frame_mask] * (1 + scale.unsqueeze(1)) + shift.unsqueeze(1)
            return x
        else:
...
def apply_gate(x, gate=None, tanh=False, condition_type=None, tr_gate=None, frist_frame_token_num=None, first_frame_mask=None):
    ...
    if condition_type == "token_replace":
        if first_frame_mask is not None:
            if gate is None:
                return x
            if tanh:
                x[:, first_frame_mask] = x[:, first_frame_mask] * tr_gate.unsqueeze(1).tanh()
                x[:, ~first_frame_mask] = x[:, ~first_frame_mask] * gate.unsqueeze(1).tanh()
                return x
            else:
                x[:, first_frame_mask] = x[:, first_frame_mask] * tr_gate.unsqueeze(1)
                x[:, ~first_frame_mask] = x[:, ~first_frame_mask] * gate.unsqueeze(1)
                return x
        else:
            if gate is None:
                return x
