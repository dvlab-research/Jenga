...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def denoise_to_zero(
        ...
        """
        Completely denoise a noisy sample to zero noise level using the model.
        
        Args:
            model_fn (`callable`):
                A function that takes a noisy sample and a timestep and returns the model output.
            noisy_sample (`torch.Tensor`):
                The initial noisy sample to be denoised.
            num_inference_steps (`int`, *optional*):
                The number of denoising steps. If not provided, will use the value set in set_timesteps.
            generator (`torch.Generator`, *optional*):
                A random number generator for stochasticity.
            return_all_samples (`bool`, defaults to `False`):
                Whether to return all intermediate samples during the denoising process.
            device (`torch.device`, *optional*):
                The device to which the timesteps should be moved.
                
        Returns:
            `torch.Tensor` or `List[torch.Tensor]`:
                If return_all_samples is False, returns the final denoised sample.
                If return_all_samples is True, returns a list of all intermediate samples.
        """
        # Set timesteps if not already set or if a new number of steps is provided
        if self.num_inference_steps is None or (num_inference_steps is not None and num_inference_steps != self.num_inference_steps):
            self.set_timesteps(num_inference_steps=num_inference_steps, device=device)
        
        # Initialize step index
        self._step_index = 0
        
        # Initialize samples list if returning all samples
        all_samples = [noisy_sample.clone()] if return_all_samples else None
        
        # Current sample starts as the noisy sample
        sample = noisy_sample
        
        # Iterate through all denoising steps
        for i, t in enumerate(self.timesteps):
            # Get model prediction for current timestep
            model_output = model_fn(sample, t)
            
            # Step the scheduler to get the next less noisy sample
            step_output = self.step(model_output, t, sample, generator=generator)
        ...
        return all_samples if return_all_samples else sample
    ...
    def step_to_zero(
        ...
        if self.config.prediction_type == "flow_prediction":
            # For flow prediction, the model predicts the flow (direction and magnitude)
            ...
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be 'flow_prediction' for the FlowDPMSolverMultistepScheduler."
            )
