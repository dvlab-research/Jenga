# with global context, add head aware selection.
...
from .attenion import attention, parallel_attention, get_cu_seqlens, my_parallel_attention
from .posemb_layers import apply_rotary_emb
from .mlp_layers import MLP, MLPEmbedder, FinalLayer
from .modulate_layers import ModulateDiT, modulate, apply_gate
from .token_refiner import SingleTokenRefiner
# from .attenion_block_flex import block_flex_attention, block_sparse_attention
# from .attention_block_triton import block_sparse_attention
# from .attention_block_fa import block_sparse_attention
from .attention_block_triton_diffres import block_sparse_attention
...
# block_sparse_attention = torch.compile(block_sparse_attention, dynamic=False)
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        # expand select_block_num for multi-GPU
        if self.hybrid_seq_parallel_attn:
            world_size = get_sequence_parallel_world_size()
            select_block_num = world_size * select_block_num
    
        if not self.hybrid_seq_parallel_attn:
            if sa_drop_rate == 0.0:
                # print("attention")
                attn = attention(
                    q, k, v, mode="flash", drop_rate=0.0, attn_mask=None, causal=True, cu_seqlens_q=cu_seqlens_q, cu_seqlens_kv=cu_seqlens_kv, max_seqlen_q=max_seqlen_q, max_seqlen_kv=max_seqlen_kv, batch_size=img_k.shape[0]
                )
            else:
                # print("block_sparse_attention")
                attn = block_sparse_attention(
                    q,
                    k,
                    v,
                    ...
                    top_k=select_block_num,#attn_mask,
                    text_amp=txt_amp,
                    block_neighbor_list=block_neighbor_list,
                    p_remain_rates=p_remain_rates,
                    text_blocks=txt_block_num
                )
        else:
            attn = my_parallel_attention(
